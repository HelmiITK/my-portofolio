...
const ExperienceComponent = () => {
  ...
  return (
    <div className="mt-7 flex flex-col gap-2">
      <h1 className="text-white font-medium text-2xl text-center" data-aos="fade-down" data-aos-duration="1000">
        Experience
      </h1>
      {displayedExperience.map((item, i) => (
        <div key={i} className="card bg-base-100 image-full max-w-sm shadow-xl mt-5 mx-4" data-aos="zoom-out" data-aos-duration="1000">
          <figure>
            <img
              src={item?.img}
              alt={item?.title} />
          ...
          <div className="card-body">
            ...
            <div className="card-actions justify-end">
              <a
                href={item?.link}
                target="_blank"
                className="bg-quaternary text-primary rounded-box py-1 px-4 text-sm">Visit</a>
            </div>
          </div>
        </div>
      ))}
